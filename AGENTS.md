...
2. **Registry** (`internal/registry`): Generator factory and plugin system
   ...
   - Built-in generators: `seq`, `pick` (values|file|url), `const`, `bool`, `int`, `float`, `uuid`, `ulid`, `time`, `regex`, `normal_int`, `normal_float`, `zipf`, `object`, `list`, `sample`, `one_of`, `template`, `switch`, `expr`, `date_offset`, `rel_ref`
